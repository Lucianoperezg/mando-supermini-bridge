...
static bool fanAddrNamespaceExists() {
    nvs_opaque_iterator_t *it = nullptr;
    if (nvs_entry_find("nvs", "fanrf", NVS_TYPE_ANY, &it) != ESP_OK || !it) {
        return false;
    }
...
static rmt_symbol_word_t    gSyms[2048]; // Optimizado de 5000 a 2048 para ahorrar RAM
static size_t               gSymN   = 0;

// ── CC1101 SPI helpers ───────────────────────────────────────
namespace cc1101 {
    static inline void selectChip() { digitalWrite(FAN_PIN_CSN, LOW); }
    static inline void deselectChip() { digitalWrite(FAN_PIN_CSN, HIGH); }

    static bool waitMisoReady(uint32_t ms = 300) {
        uint32_t t = millis();
    ...
    static void writeRegister(uint8_t a, uint8_t v) {
        selectChip();
        if (!waitMisoReady()) { deselectChip(); return; }
        SPI.transfer(a);
        SPI.transfer(v);
        deselectChip();
    }

    static uint8_t readStatusRegister(uint8_t a) {
        selectChip();
        if (!waitMisoReady()) { deselectChip(); return 0xFF; }
        SPI.transfer(a | 0xC0);
    ...
    static void writeBurst(uint8_t a, const uint8_t* d, size_t n) {
        selectChip();
        if (!waitMisoReady()) { deselectChip(); return; }
        SPI.transfer(a | 0x40);
    ...
    static void sendStrobe(uint8_t c) {
        selectChip();
        if (!waitMisoReady()) { deselectChip(); return; }
        SPI.transfer(c);
        deselectChip();
...
bool fanRfInit(){
    ...
    // Reset sequence
    cc1101::deselectChip(); delayMicroseconds(5);
    cc1101::selectChip();   delayMicroseconds(10);
    cc1101::deselectChip(); delayMicroseconds(40);
    cc1101::selectChip();   if(!cc1101::waitMisoReady()){cc1101::deselectChip();return false;}
    SPI.transfer(CC1101_SRES);
    if(!cc1101::waitMisoReady()){cc1101::deselectChip();return false;}
    cc1101::deselectChip(); delay(20);

    uint8_t pn = cc1101::readStatusRegister(0x30); // PARTNUM
    uint8_t vv = cc1101::readStatusRegister(0x31); // VERSION
    if(pn!=0x00 || vv!=0x14){
        Serial.printf("[CC1101] ERR PARTNUM=0x%02X VERSION=0x%02X\n",pn,vv);
        return false;
    }

    for(auto& r : kCC1101Regs) cc1101::writeRegister(r.addr, r.val);

    // PATABLE: [0]=off [1]=PA level
    uint8_t pa[2] = {0x00, gFanTx.paValue};
    cc1101::writeBurst(0x3E, pa, 2);

    cc1101::sendStrobe(CC1101_SIDLE); delay(5);
    cc1101::sendStrobe(CC1101_SCAL);  delay(50);
...
static bool transmit(){
    ...
    if(e!=ESP_OK){
        cc1101::sendStrobe(CC1101_SIDLE);
...
void fanLoadAddr(){
    if (!fanAddrNamespaceExists()) {
        gAddrCur = FAN_ID_NUEVO;
        return;
    }
...
void fanPairBruteForce(uint32_t addr, uint32_t dwellMs){
